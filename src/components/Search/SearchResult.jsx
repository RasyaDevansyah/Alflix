      ...
      {result.length === 0 ? (
        ...
        result.map((movie, idx) => (
          <Link
            ...
            className='flex flex-row items-center w-full h-14 bg-gray-800 rounded-lg mt-2 shadow-lg px-4'
          >
            <span className='flex w-full items-center text-white font-bold hover:text-purple-400 hover:underline hover:bg-gray-700 rounded-lg'>
              <img src={movie.poster} alt={movie.title} className='w-10 h-10 rounded-lg mr-2' />
              {movie.title}
            </span>
          </Link>
        ))
